                        ...
                        {camps.map((item, index) => (
                            <tr key={item._id} className="hover:bg-gray-50">
                                ...
                                <td className="border border-gray-300 px-4 py-2">
                                    {item.confirmation_status === 'Confirmed' || item.confirmation_status === 'Rejected' ? (<button disabled className="btn btn-sm btn-primary">{item.confirmation_status}</button>) : (<button onClick={() => handleChange(item._id, item.confirmation_status, 'Confirmed')} className="btn btn-sm btn-primary hover:underline">{item.confirmation_status}</button>)}
                                </td>
